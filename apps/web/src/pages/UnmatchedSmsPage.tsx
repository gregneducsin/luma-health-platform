        ...
        <div className="space-y-3 border-t border-gray-100">
          <div className="px-4 pt-3">
            ...
            <p className="text-sm text-gray-800">
              {thread.fromName ? `${thread.fromName} (${thread.fromPhone})` : thread.fromPhone}
              {thread.collectedEmail && ` · ${thread.collectedEmail}`}
            </p>
          </div>
          ...
          {thread.linkedCustomerId && (
            <div className="mx-4 rounded-md bg-green-50 px-3 py-2">
              <p className="text-xs text-green-800">
                ...
                or the Conversations tab for that reply. Every text from this number from now on routes through Lucy automatically, same as any other lead.
              </p>
            </div>
          )}
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-400">
        Inbound texts from a phone number that doesn't match any customer record, grouped one thread per number. On the first message, a fixed,
        content-free acknowledgment goes out immediately asking for their name, so nobody sits in silence — that's the only thing sent with no
        review. Beyond that, Claude drafts a classification and a suggested reply for staff to review and send or dismiss — asking for an email
        next once a name is known, since a text never comes with one attached. Suggested matches to an existing customer are never linked
        automatically. Once both a name and a real-looking email are known and Claude is confident it's a genuine new lead, a lead is created
